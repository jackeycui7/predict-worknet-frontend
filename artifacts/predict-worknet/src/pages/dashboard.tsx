...
export default function Dashboard() {
  ...
  return (
    <div className="animate-fade-up">
      <div className="mb-16">
        <h1 className="font-serif-editorial text-[72px] leading-[1.05] tracking-[-0.03em] text-foreground">
      ...
      <div className="grid grid-cols-5 border-t border-border/60 mb-16">
      ...
      <div className="grid grid-cols-[1fr_340px] gap-12 mb-16">
        <div>
          ...
          <div className="border-t border-border/60">
            {feed?.map((item, i) => (
              <div
                key={`feed-${i}`}
                className={`py-2.5 border-b border-border/40 text-[12px] flex items-center gap-3 hover:bg-foreground/[0.02] transition-colors ${i < 3 ? "animate-feed-slide" : ""}`}
              >
            ...
            {(!feed || feed.length === 0) && (
              <div className="py-12 text-center text-foreground/20 text-[13px] font-light">
...
function MetricCell({ label, value, sub, subColor, last }: { label: string; value: string; sub?: string; subColor?: string; last?: boolean }) {
  return (
    <div className={`py-6 pr-6 ${!last ? "border-r border-border/40" : ""}`}>
      <div className="text-[10px] font-light text-foreground/25 tracking-[0.06em] uppercase mb-3">{label}</div>
      <div className="font-serif-editorial text-[36px] tracking-[-0.02em] text-foreground leading-[1] mb-1">{value}</div>
